  ...
  try {
    ...
    if (ANSWERS.respuesta1 === "Si") {
      if (!req.body.organizaciones_sociales) {
        return res.status(400).json({
          status: false,
          statusText: "Escriba las organizaciones sociales a las que pertenéce",
        });
      }
    ...
    if (ANSWERS.respuesta2 === "Si") {
      if (!req.body.organismos_profesionistas) {
        return res.status(400).json({
          status: false,
          statusText:
            "Escriba los organismos profesionistas a los que pertenéce",
        });
      }
    ...
    if (
      ...
      if (process.env.OS === "unix") {
        ...
        html = fs.readFileSync(
          path.join(__dirname, "../assets/template.unix.html"),
          "utf8"
        );
      }
      const options = {
      ...
      const QR = await qrCode.toDataURL(
        `${process.env.HOST}/survey/verify/${token}/`
      );
